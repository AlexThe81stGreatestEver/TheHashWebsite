  ...
  protected function getHaringPercentageByHareTypeQuery() {
    return
      "SELECT HASHERS.HASHER_NAME AS HASHER_NAME,
	      ...
	      ((HARING_COUNT_TEMP_TABLE.HARING_COUNT / HASH_COUNT_TEMP_TABLE.HASH_COUNT) * 100) AS VALUE,
	      (SELECT MAX(HASHES.EVENT_DATE)
		 FROM HASHES
                 JOIN HASHINGS
		   ON HASHINGS.HASHER_KY = HASHERS.HASHER_KY
                  AND HASHINGS.HASH_KY = HASHES.HASH_KY
                WHERE HASHES.KENNEL_KY = ?) AS LATEST_EVENT
         FROM ((HASHERS
