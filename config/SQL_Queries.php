...
define ("SELECT_HASHERS_VIRGIN_HARE","SELECT
	HASHES.HASH_KY,
    KENNEL_KY,
    KENNEL_EVENT_NUMBER,
    EVENT_DATE,
...
define ("SELECT_HASHERS_MOST_RECENT_HARE","SELECT
	HASHES.HASH_KY,
    KENNEL_KY,
    KENNEL_EVENT_NUMBER,
    EVENT_DATE,
...
define("KENNEL_NUM_OF_DISTINCT_HARES","SELECT COUNT(DISTINCT(HARINGS.HARINGS_HASHER_KY)) AS THE_COUNT
	FROM HASHES JOIN HARINGS ON HASHES.HASH_KY = HARINGS.HARINGS_HASH_KY
	WHERE HASHES.KENNEL_KY = ? AND HARINGS.HARE_TYPE & ? != 0");
...
DEFINE("VIRGIN_HARINGS_BY_YEAR","SELECT DATE_FORMAT(FIRST_HARING_DATE,'%Y') AS THE_VALUE, COUNT(*) AS THE_COUNT FROM (
	SELECT
		...
		FIRST_HARING_EVENT_TABLE.FIRST_HASH_DATE AS FIRST_HARING_DATE,
                (SELECT HASH_KY FROM HASHES WHERE KENNEL_KY=? AND EVENT_DATE=FIRST_HASH_DATE) AS FIRST_HARING_KEY
	FROM
		(HASHERS
		JOIN (
			SELECT
...
DEFINE("STREAKERS_LIST","
    WITH THIS_EVENT AS (
         ...
         SELECT HASHER_KY, seq-(CAST(row_number() over(order by HASHER_KY, seq) AS SIGNED)) as grp
           FROM (SELECT CAST(seq AS SIGNED) AS seq, HASHER_KY, HASHINGS.HASH_KY
                  ...
                  WHERE HASHER_KY IN (SELECT HASHER_KY FROM HASHINGS WHERE HASH_KY=(SELECT HASH_KY FROM THIS_EVENT))) AS hg),
    ...
    SELECT HASHER_KY AS THE_HASHER_KY, (SELECT HASHER_NAME FROM HASHERS WHERE HASHERS.HASHER_KY = grps.HASHER_KY) AS THE_HASHER_NAME, grp_count AS THE_STREAK_IN_EVENTS,
           (SELECT COALESCE((
           SELECT DATE_FORMAT(EVENT_DATE, '%Y/%m/%d') FROM HASHES WHERE HASH_KY=(
           SELECT HASH_KY FROM ALL_EVENTS WHERE seq=(SELECT seq FROM ALL_EVENTS JOIN THIS_EVENT ON THIS_EVENT.HASH_KY = ALL_EVENTS.HASH_KY) - grp_count)), 'NA')) AS LAST_MISSED_EVENT,
           1+DATEDIFF((SELECT EVENT_DATE FROM THIS_EVENT),
           (SELECT EVENT_DATE FROM HASHES WHERE HASH_KY=(
           SELECT HASH_KY FROM ALL_EVENTS WHERE seq=(SELECT seq FROM ALL_EVENTS JOIN THIS_EVENT ON THIS_EVENT.HASH_KY = ALL_EVENTS.HASH_KY) - grp_count + 1))) AS THE_STREAK_IN_DAYS
      FROM (
            SELECT HASHER_KY, COUNT(*) AS grp_count
...
DEFINE("HASHER_ATTENDANCE_RECORD_LIST","SELECT
	CASE
        ...
        ELSE 'YES'
	END AS ATTENDED,
    HASHES.*, HASH_TYPE_NAME FROM (
		SELECT HASHINGS.HASH_KY, HASHES.EVENT_DATE, HASH_TYPES.HASH_TYPE_NAME
		FROM HASHINGS JOIN HASHES ON HASHINGS.HASH_KY = HASHES.HASH_KY
